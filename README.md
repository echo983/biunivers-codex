...
- Each user turn is limited to eight tool calls. Provider failures use a short backoff and are attempted at most three times before becoming a visible terminal answer. A reasoning-only response triggers an explicit output-repair instruction on the bounded retries, preventing nested retry storms without blindly repeating the same incomplete request.
